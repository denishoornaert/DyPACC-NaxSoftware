...
void countnegative_initialize(matrix Array)
{
  ...
  _Pragma("loopbound min 128 max 128") for (OuterIndex = 0; OuterIndex < MAXSIZE; OuterIndex++)
      _Pragma("loopbound min 128 max 128") for (InnerIndex = 0; InnerIndex < MAXSIZE; InnerIndex++)
...
void countnegative_sum(matrix Array)
{
  ...
  _Pragma("loopbound min 128 max 128") for (Outer = 0; Outer < MAXSIZE; Outer++)
      _Pragma("loopbound min 128 max 128") for (Inner = 0; Inner < MAXSIZE; Inner++) if (Array[Outer][Inner] >= 0)
  {
