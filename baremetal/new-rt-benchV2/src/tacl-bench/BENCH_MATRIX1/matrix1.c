...
void matrix1_pin_down(int A[], int B[], int C[])
{
  ...
  _Pragma("loopbound min 100 max 100") for (i = 0; i < X * Y; i++)
      A[i] = x;

  _Pragma("loopbound min 100 max 100") for (i = 0; i < Y * Z; i++)
      B[i] = x;

  _Pragma("loopbound min 100 max 100") for (i = 0; i < X * Z; i++)
      C[i] = 0;
}
...
void benchmark_execution(int parameters_num, void **parameters)
{
  ...
  _Pragma("loopbound min 10 max 10") for (k = 0; k < Z; k++)
  {
    ...
    _Pragma("loopbound min 10 max 10") for (i = 0; i < X; i++)
    {
      ...
      *p_c = 0;
      _Pragma("loopbound min 10 max 10") for (f = 0; f < Y; f++) /* do multiply */
          *p_c += *p_a++ * *p_b++;
