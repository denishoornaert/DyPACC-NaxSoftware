...
// OLD: int bitonic_a[ 32 ];         // the array to be sorted
int bitonic_a[262144]; // the array to be sorted
...
int benchmark_init(int parameters_num, void **parameters)
{
  ...
  _Pragma("loopbound min 262144 max 262144") for (i = 0; i < 262144; i++)
      bitonic_a[i] = (262144 - i);
...
void bitonic_merge(int lo, int cnt, int dir)
{
  int k = cnt / 2;
  int i;
  _Pragma("loopbound min 0 max 16") for (i = lo; i < lo + k; i++)
...
void benchmark_execution(int parameters_num, void **parameters)
{
  ...
  _Pragma("marker recSort")
      bitonic_sort(0, 262144, bitonic_ASCENDING);
