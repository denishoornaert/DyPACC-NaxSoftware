...
#define bsort_SIZE (32 * 1024)
// OLD: #define bsort_SIZE 100
...
int bsort_Initialize(int Array[])
{
  ...
  _Pragma("loopbound min 32768 max 32768") for (Index = 0; Index < bsort_SIZE; Index++)
...
int bsort_return(void)
{
  ...
  _Pragma("loopbound min 99 max 99") for (Index = 0; Index < bsort_SIZE - 1; Index++)
