...
extern unsigned int quicksort_input_vector[1000 * 3 * 8];
struct quicksort_3DVertexStruct quicksort_vectors[1000 * 8];
...
int benchmark_init(int parameters_num, void **parameters)
{
  ...
  _Pragma("loopbound min 24000 max 24000") for (i = 0; i < 24000; i++)
      quicksort_input_vector[i] += quicksort_const_prop_border_i;

  /* Init arrays */
  _Pragma("loopbound min 681 max 681") for (i = 0; i < 681; i++)
  {
    _Pragma("loopbound min 1 max 20") for (j = 0; j < 20 - 1; j++)
    {
  ...
  _Pragma("loopbound min 8000 max 8000") for (i = 0; i < 8000; i++)
  {
...
void quicksort_str(char *a, unsigned long n, unsigned long es)
{
  ...
  _Pragma("loopbound min 0 max 8") while (n > 1)
  {
    if (n > 10)
    ...
    _Pragma("loopbound min 0 max 169") while (1)
    {
      _Pragma("loopbound min 1 max 26") do pi += es;
      while ((pi < pn) && (quicksort_compare_strings(pi, a) < 0))
        ;

      _Pragma("loopbound min 1 max 23") do pj -= es;
...
void quicksort_vec(char *a, unsigned long n, unsigned long es)
{
  ...
  _Pragma("loopbound min 0 max 15") while (n > 1)
  {
    if (n > 10)
    ...
    _Pragma("loopbound min 1 max 250") while (1)
    {
      _Pragma("loopbound min 1 max 51") do pi += es;
      while ((pi < pn) && (quicksort_compare_vectors(pi, a) < 0))
        ;

      _Pragma("loopbound min 1 max 27") do pj -= es;
