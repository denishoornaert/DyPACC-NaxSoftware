...
void sha_transform(struct SHA_INFO *sha_info)
{
  ...
  _Pragma("loopbound min 16 max 16") for (i = 0; i < 16; ++i)
      W[i] = sha_info->data[i];
  _Pragma("loopbound min 64 max 64") for (i = 16; i < 80; ++i)
  ...
  _Pragma("loopbound min 20 max 20") for (i = 0; i < 20; ++i)
  {
    FUNC(1, i);
  }
  _Pragma("loopbound min 20 max 20") for (i = 20; i < 40; ++i)
  {
    FUNC(2, i);
  }
  _Pragma("loopbound min 20 max 20") for (i = 40; i < 60; ++i)
  {
    FUNC(3, i);
  }
  _Pragma("loopbound min 20 max 20") for (i = 60; i < 80; ++i)
  {
    FUNC(4, i);
  }
  sha_info->digest[0] += A;
                 ...
                 struct SHA_MY_FILE *stream)
{
  ...
  _Pragma("loopbound min 0 max 8192") while (i < stream->cur_pos + number_of_chars_to_read)((unsigned char *)ptr)[i2++] = stream->data[i++];
...
void sha_stream(struct SHA_INFO *sha_info, struct SHA_MY_FILE *fin)
{
  ...
  _Pragma("loopbound min 5 max 5") while ((i = sha_fread(data, 1, BLOCK_SIZE, fin)) > 0)
...
void benchmark_execution(int parameters_num, void **parameters)
{
  ...
  fin.data = sha_data;
  fin.size = 32743 * 8; // set size = 3247552 for input_large
  fin.cur_pos = 0;
