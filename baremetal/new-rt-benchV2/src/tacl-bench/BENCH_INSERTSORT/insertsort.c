...
*/
unsigned int insertsort_a[32768];
...
void insertsort_initialize(unsigned int *array)
{
  ...
  insertsort_a[0] = 0;
  _Pragma("loopbound min 32768 max 32768") for (i = 1; i < 32768; i++)
      insertsort_a[i] = 32768 - i;
}

unsigned int a[32768] = {0, 11, 10, 9, 8, 7, 6, 5, 4, 3, 2};
...
void benchmark_execution(int parameters_num, void **parameters)
{
  ...
  _Pragma("loopbound min 32768 max 32768") while (i <= 32767)
  {
    ...
    _Pragma("loopbound min 1 max 32768") while (insertsort_a[j] < insertsort_a[j - 1])
    {
      insertsort_iters_a++;
