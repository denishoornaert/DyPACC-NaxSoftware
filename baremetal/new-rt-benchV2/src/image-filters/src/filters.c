...
int apmax(int a, int b) { return (a > b) ? a : b; }
int apmin(int a, int b) { return (a < b) ? a : b; }

// prio
void threshold(struct bmp_t *img)
{
    for (u32 h = 0; h < img->height; h++)
        for (u32 r = 0; r < img->width * img->depth; r++)
...
//     for (u32 h = 0; h < img->height; h++)
//     {
//         for (u32 p = 0; p < img->width * img->depth; p += img->depth)
//         {
...
void grayscale(struct bmp_t *img, struct bmp_t *out)
{
    for (u32 h = 0; h < img->height; h++)
    {
        for (u32 p = 0; p < img->width * img->depth; p += img->depth)
        {
            u32 weighted_sum = ((unsigned char)img->data[h][p + 2] * 2989 / 10000) + ((unsigned char)img->data[h][p + 1] * 5870 / 10000) + ((unsigned char)img->data[h][p + 0] * 1140 / 10000);
            char gray = (weighted_sum >= 255) ? 255 : weighted_sum;
            out->data[h][p + 0] = gray;
            out->data[h][p + 1] = gray;
            out->data[h][p + 2] = gray;
...
void sepia(struct bmp_t *img, struct bmp_t *out)
{
    for (u32 h = 0; h < img->height; h++)
    {
        for (u32 p = 0; p < img->width * img->depth; p += img->depth)
        {
            int r = (unsigned char)img->data[h][p + 2];
            int g = (unsigned char)img->data[h][p + 1];
            int b = (unsigned char)img->data[h][p + 0];
...
void sobel(struct bmp_t *img, struct bmp_t *out, unsigned size, int kh[size][size], int kv[size][size])
{
    ...
    for (u32 h = half_size; h < img->height - half_size; h++)
    {
        for (u32 p = img->depth * half_size; p < (img->width - half_size) * img->depth; p += img->depth)
        {
            ...
            char color = apmin(apmax(sqrt(gx * gx + gy * gy), 0), 255);
...
//     for (u32 h = half_size; h < img->height - half_size; h++)
//     {
//         for (u32 r = img->depth * half_size; r < (img->width - half_size) * img->depth; r++)
//         {
//             int res = 0;
//             for (u32 i = 0; i < size; i++)
//             {
...
void gaussian_noise(struct bmp_t *img, struct bmp_t *out, unsigned size, float conv[size][size])
{
    int conv_approx[size][size];
    for (u32 x = 0; x < size; x++)
    {
        for (u32 y = 0; y < size; y++)
            conv_approx[x][y] = conv[x][y] * 1000000;
    }
    ...
    for (u32 h = half_size; h < img->height - half_size; h++)
    {
        for (u32 r = img->depth * half_size; r < (img->width - half_size) * img->depth; r++)
        {
            ...
            for (u32 i = 0; i < size; i++)
            {
                for (u32 j = 0; j < size; j++)
                {
                    res += img->data[h + (i - half_size)][r + ((j - half_size) * img->depth)] * conv_approx[i][j] / 1000000;
                }
            }
            out->data[h][r] = apmin(apmax(res, 0), 255);
        }
    }
}
...
{
    for (u32 h = 1; h < img->height - 1; h++)
    {
        for (u32 p = 1; p < img->width - 1; p++)
        {
            // If negative, add PI
...
void edge_tracking(struct bmp_t *img, struct bmp_t *out)
{
    ...
    for (u32 h = 1; h < img->height - 1; h++)
    {
        for (u32 p = img->depth; p < (img->width - 1) * img->depth; p += img->depth)
        {
            uint8_t neighbourhood = 0;
            for (u32 i = 0; (i < size) & (neighbourhood != 255); i++)
            {
                for (u32 j = 0; (j < size) & (neighbourhood != 255); j++)
                {
                    if (i != j)
                    {
